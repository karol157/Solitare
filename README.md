...
Features a rich TUI, keyboard/mouse controls, undo, auto-win, music, and multi-language support.

---

## Features

- Classic Solitaire (Klondike) gameplay
- Keyboard and mouse support
- Undo last moves
- Auto-win mode
- Configurable settings (music, language, difficulty, etc.)
- English and Polish translations
- Animated card rendering in the terminal
- Sound/music playback (WAV files)
...
- **Auto Win**: Automatically finish the game if possible
...
3. **Run the game**  
   ...
   python main.py
   ```
   Or use the provided `luncher.py` for auto-setup on Linux.
...
- `game/`  
  Main game logic, UI, and modules:
...
Settings are stored in `game/config/settings.json` and can be changed via the in-game settings screen.
...
- Card rendering and logic by project author
